...
const pageCss = "\n        body { background-color: #F9F6F0; } /* parchment-white */\n        .wax-seal-shadow { box-shadow: 0 4px 12px rgba(139, 0, 0, 0.15); }\n        .debossed-input { border: 1px inset #e6d7d5; background-color: #F0EDE4; }\n        \n        /* Custom radio button styling for luxury feel */\n        input[type=\"radio\"].luxury-radio:checked + label {\n            background-color: #4A0E0E; /* deep-burgundy */\n            color: #C5A059; /* antique-gold */\n            border-color: #4A0E0E;\n        }\n    ";

export const Route = createFileRoute("/_authenticated/diagnostico/inicial")({
  head: () => ({
    meta: [
      { title: "Diagnóstico Inicial — Passaporte Capilar™" },
      { name: "description", content: "Etapa inicial do diagnóstico capilar com avaliação guiada do profissional." },
      { property: "og:title", content: "Diagnóstico Inicial — Passaporte Capilar™" },
      { property: "og:description", content: "Etapa inicial do diagnóstico capilar com avaliação guiada do profissional." },
  ...
  return (
    <div className="font-body-lg text-on-surface bg-parchment-white antialiased selection:bg-antique-gold selection:text-deep-burgundy">
      ...
      <main className="ml-0 md:ml-64 pt-16 md:pt-24 px-4 md:px-margin-desktop min-h-screen flex flex-col pb-24">
        <div className="mb-10 flex flex-col items-center max-w-4xl mx-auto w-full">
          ...
          <div className="w-full flex items-center gap-2 mb-6">
            ...
            <div className="font-label-caps text-label-caps text-deep-burgundy px-4 py-1 border border-antique-gold/50 rounded-full bg-surface">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-gutter max-w-7xl mx-auto w-full">
          <div className="lg:col-span-8 space-y-chapter-gap">
            <section className="bg-surface border-b border-outline-variant pb-10 relative">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div>
                  ...
                  <div className="flex gap-2">
                    ...
                    <label className="flex-1 text-center py-3 border border-outline-variant rounded cursor-pointer transition-all font-body-sm text-body-sm text-on-surface hover:border-deep-burgundy" htmlFor="por-baixa">
                      Baixa (Resistente)
                    </label>
                    <input defaultChecked={true} className="hidden luxury-radio" id="por-media" name="porosity" type="radio" />
                    <label className="flex-1 text-center py-3 border border-outline-variant rounded cursor-pointer transition-all font-body-sm text-body-sm text-on-surface hover:border-deep-burgundy" htmlFor="por-media">
                      Média (Normal)
                    </label>
                    <input className="hidden luxury-radio" id="por-alta" name="porosity" type="radio" />
                    <label className="flex-1 text-center py-3 border border-outline-variant rounded cursor-pointer transition-all font-body-sm text-body-sm text-on-surface hover:border-deep-burgundy" htmlFor="por-alta">
                      Alta (Danificado)
                    </label>
                  </div>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    <label className="text-center py-3 border border-outline-variant rounded cursor-pointer transition-all font-body-sm text-body-sm text-on-surface hover:border-deep-burgundy" htmlFor="elas-excelente">
                      Excelente (Retorna)
                    </label>
                    <input defaultChecked={true} className="hidden luxury-radio" id="elas-boa" name="elasticity" type="radio" />
                    <label className="text-center py-3 border border-outline-variant rounded cursor-pointer transition-all font-body-sm text-body-sm text-on-surface hover:border-deep-burgundy" htmlFor="elas-boa">
                      Boa (Lenta)
                    </label>
                    <input className="hidden luxury-radio" id="elas-ruim" name="elasticity" type="radio" />
                    <label className="text-center py-3 border border-outline-variant rounded cursor-pointer transition-all font-body-sm text-body-sm text-on-surface hover:border-deep-burgundy" htmlFor="elas-ruim">
                      Baixa (Estica e não volta)
                    </label>
                    <input className="hidden luxury-radio" id="elas-critica" name="elasticity" type="radio" />
                    <label className="text-center py-3 border border-outline-variant rounded cursor-pointer transition-all font-body-sm text-body-sm text-on-surface hover:border-deep-burgundy" htmlFor="elas-critica">
                      Crítica (Rompe)
                    </label>
                  </div>
                </div>
            ...
            <section className="bg-surface border-b border-outline-variant pb-10 relative mt-12">
              <div className="absolute top-0 right-0 font-headline-lg text-headline-lg text-antique-gold/20 select-none">
                02
              </div>
              <h3 className="font-title-md text-title-md text-deep-burgundy mb-6 flex items-center gap-2">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <label className="flex flex-col items-center justify-center p-4 border border-outline-variant rounded cursor-pointer hover:bg-surface-container-low transition-colors group">
                  <input defaultChecked={true} className="accent-deep-burgundy w-4 h-4 mb-3" type="checkbox" />
                  <span className="font-body-sm text-body-sm font-medium text-center group-hover:text-deep-burgundy">
                    Normal / Saudável
                  </span>
                </label>
                <label className="flex flex-col items-center justify-center p-4 border border-outline-variant rounded cursor-pointer hover:bg-surface-container-low transition-colors group">
            ...
            <section className="bg-surface pb-10 relative mt-12">
              <div className="absolute top-0 right-0 font-headline-lg text-headline-lg text-antique-gold/20 select-none">
                03
              </div>
              <h3 className="font-title-md text-title-md text-deep-burgundy mb-6 flex items-center gap-2">
              ...
              <div className="relative">
                ...
                <textarea className="w-full debossed-input p-4 rounded-md font-body-lg text-body-lg text-on-surface focus:outline-none focus:ring-1 focus:ring-deep-burgundy resize-none" id="tech-notes" placeholder="Detalhes adicionais sobre histórico químico recente, incompatibilidades ou notas para a execução..." rows={4}></textarea>
                <div className="absolute bottom-3 right-3 text-metadata font-metadata text-on-surface-variant">
                  Carimbo de Registro: Dr. Thay
                </div>
              </div>
            </section>
            <div className="flex justify-end gap-4 mt-8 pt-8 border-t border-outline-variant">
              ...
              <button className="px-8 py-3 bg-deep-burgundy text-antique-gold font-label-caps text-label-caps uppercase rounded shadow-[0_4px_12px_rgba(74,14,14,0.2)] hover:bg-primary transition-all flex items-center gap-2 group">
                Validar Diagnóstico
          ...
          <aside className="lg:col-span-4 space-y-6">
            <div className="bg-parchment-white border border-outline-variant rounded-lg p-6 relative overflow-hidden shadow-sm">
              ...
              <div className="flex items-center gap-4 border-b border-outline-variant pb-4 mb-4">
                <div className="w-16 h-16 rounded-full overflow-hidden border-2 border-antique-gold">
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <div className="bg-surface-container-low p-3 rounded text-sm text-on-surface-variant border-l-2 border-antique-gold">
                    Porosidade: Alta
                    <br />
            ...
            <div className="rounded-lg overflow-hidden border border-outline-variant shadow-sm bg-surface">
